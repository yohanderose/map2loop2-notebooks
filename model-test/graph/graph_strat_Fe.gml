Creator "map2model-cpp"
graph [
  hierarchic 1
  directed 1
  node [
    id -3
    LabelGraphics [ text "Fortescue Group" anchor "n" fontStyle "bold" fontSize 14 ]
    isGroup 1
    graphics [ fill "#FAFAFA" ]
  ]
  node [
    id -1
    LabelGraphics [ text "Hamersley Group" anchor "n" fontStyle "bold" fontSize 14 ]
    isGroup 1
    graphics [ fill "#FAFAFA" ]
  ]
  node [
    id -9
    LabelGraphics [ text "Turee Creek Group" anchor "n" fontStyle "bold" fontSize 14 ]
    isGroup 1
    graphics [ fill "#FAFAFA" ]
  ]
  node [
    id 4
    LabelGraphics [ text "Boolgeeda Iron Formation" fontSize 14 ]
    gid -1
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 1
    LabelGraphics [ text "Brockman Iron Formation[2]" fontSize 14 ]
    gid -1
    graphics [ fill "#ddffdd" w 150 ]
  ]
  node [
    id 11
    LabelGraphics [ text "Bunjinah Formation" fontSize 14 ]
    gid -3
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 7
    LabelGraphics [ text "Fortescue Group" fontSize 14 ]
    gid -3
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 2
    LabelGraphics [ text "Jeerinah Formation" fontSize 14 ]
    gid -3
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 9
    LabelGraphics [ text "Marra Mamba Iron Formation" fontSize 14 ]
    gid -1
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 5
    LabelGraphics [ text "Mount McRae Shale and Mount Sylvia Formation" fontSize 14 ]
    gid -1
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 0
    LabelGraphics [ text "Pyradie Formation" fontSize 14 ]
    gid -3
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 10
    LabelGraphics [ text "Turee Creek Group" fontSize 14 ]
    gid -9
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 8
    LabelGraphics [ text "Weeli Wolli Formation" fontSize 14 ]
    gid -1
    graphics [ fill "#ffffff" w 150 ]
  ]
  node [
    id 6
    LabelGraphics [ text "Wittenoom Formation[1]" fontSize 14 ]
    gid -1
    graphics [ fill "#eeffee" w 150 ]
  ]
  node [
    id 3
    LabelGraphics [ text "Woongarra Rhyolite" fontSize 14 ]
    gid -1
    graphics [ fill "#ffffff" w 150 ]
  ]
  edge [
    source 11
    target 0
    graphics [ style "line" arrow "last" width 5 fill "#00649a" ]
  ]
  edge [
    source 1
    target 5
    graphics [ style "line" arrow "last" width 5 fill "#002dd1" ]
    LabelGraphics [ text "15" fill "#00ff00" fontSize 14 fontStyle "bold" model "centered" position "center" outline "#000000"]
  ]
  edge [
    source 1
    target 6
    graphics [ style "line" arrow "last" width 3 fill "#fc0200" ]
  ]
  edge [
    source 7
    target 0
    graphics [ style "line" arrow "last" width 3 fill "#0000ff" ]
  ]
  edge [
    source 2
    target 7
    graphics [ style "line" arrow "last" width 5 fill "#0025d9" ]
  ]
  edge [
    source 9
    target 2
    graphics [ style "line" arrow "last" width 5 fill "#001de1" ]
    LabelGraphics [ text "2" fill "#ddffdd" fontSize 14 fontStyle "bold" model "centered" position "center" outline "#000000"]
  ]
  edge [
    source 5
    target 2
    graphics [ style "line" arrow "last" width 1 fill "#956900" ]
  ]
  edge [
    source 2
    target 11
    graphics [ style "line" arrow "last" width 5 fill "#0046b8" ]
  ]
  edge [
    source 4
    target 3
    graphics [ style "line" arrow "last" width 5 fill "#0015e9" ]
  ]
  edge [
    source 3
    target 8
    graphics [ style "line" arrow "last" width 5 fill "#0027d7" ]
  ]
  edge [
    source 10
    target 3
    graphics [ style "line" arrow "last" width 1 fill "#ef0f00" ]
  ]
  edge [
    source 5
    target 6
    graphics [ style "line" arrow "last" width 5 fill "#0028d6" ]
    LabelGraphics [ text "14" fill "#11ff11" fontSize 14 fontStyle "bold" model "centered" position "center" outline "#000000"]
  ]
  edge [
    source 6
    target 2
    graphics [ style "line" arrow "last" width 1 fill "#f50900" ]
  ]
  edge [
    source 4
    target 10
    graphics [ style "line" arrow "last" width 5 fill "#0016e8" ]
  ]
  edge [
    source 6
    target 9
    graphics [ style "line" arrow "last" width 5 fill "#001be3" ]
    LabelGraphics [ text "10" fill "#55ff55" fontSize 14 fontStyle "bold" model "centered" position "center" outline "#000000"]
  ]
  edge [
    source 5
    target 9
    graphics [ style "line" arrow "last" width 3 fill "#00bf3f" ]
    LabelGraphics [ text "2" fill "#ddffdd" fontSize 14 fontStyle "bold" model "centered" position "center" outline "#000000"]
  ]
  edge [
    source 8
    target 1
    graphics [ style "line" arrow "last" width 5 fill "#0030ce" ]
    LabelGraphics [ text "6" fill "#99ff99" fontSize 14 fontStyle "bold" model "centered" position "center" outline "#000000"]
  ]
]